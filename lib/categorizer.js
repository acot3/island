...
5. Whether the action is *addressed to* a specific character known to be present at this location. The list of present characters (if any) is given in <present-characters> below.
   - If the action talks to, asks, offers to, attacks, or otherwise directly engages a present character, return that character's KEY (e.g. "krab").
   - If the action is not directed at any present character — even if a character happens to be there and the player is doing something else — return null.
...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      addressing: {
        type: ['string', 'null'],
        description: "If the action is directed at a specific character known to be present, the character's key. Else null.",
      },
      rationale: {
        type: 'string',
    ...
    required: ['possible', 'attribute', 'difficulty', 'seeking', 'addressing', 'rationale'],
  },
};
