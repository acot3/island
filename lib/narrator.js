...
- A <character-replies> block may be present. For each entry there, embed the verbatim "reply" text as a segment with the given voice key, slotted naturally inside the paragraph covering that player's location. Wrap it in the prose: a narrator segment of setup → the character segment (verbatim) → optionally a narrator segment of follow-up.
- Never modify the character reply text. Use it exactly as given.
- Never put character dialogue inside a narrator segment.

COVERAGE — NON-NEGOTIABLE
- Every player listed in <action-reports> MUST appear in your prose. Skipping a player is not allowed under any circumstance. Even when their action was IMPOSSIBLE, or their day was quiet, or another player's scene is more dramatic — write their paragraph.
- Every entry in <character-replies> MUST appear as a verbatim character segment.
- A useful self-check before you emit: list every player in <action-reports> and confirm each one is somewhere in your prose. If anyone is missing, rewrite.`;
    ...
    input_schema: {
      ...
      properties: {
        segments: {
          ...
          description: 'Ordered list of voice segments. Use "narrator" for prose. For any character-reply provided in <character-replies>, emit the verbatim reply as a segment with the character\'s voice key.',
        },
      },
      required: ['segments'],
    },
...
async function narrateDay({ narrative, day, players, locations, actionReports, sceneHandoff, characterReplies }) {
...
HINT: ${sceneHandoff.hint || 'a short transitional sentence pointing the affected player(s) toward where their separate scene will begin.'}
    ...
    : '';
  const repliesBlock = (characterReplies && characterReplies.length)
    ? `\n\n<character-replies>
${characterReplies.map((r) => `- ${r.playerName} addressed ${r.characterName} at ${place(tokens, r.nodeId, r.biome)}.
  Player's action: "${r.action}"
  ${r.characterName}'s verbatim reply (voice key: "${r.voice}"):
  "${r.reply}"`).join('\n')}
</character-replies>
For each entry above, embed the verbatim reply as a segment with the given voice key inside the paragraph covering that player's location. Surround it naturally with narrator setup and follow-up.`
    : '';
...
</action-reports>${repliesBlock}${handoffBlock}
...
Write the after-action prose for Day ${day}, dramatizing what just happened. Continue the existing story; do not recap.`;
  ...
  // characterReplies. The tool's schema validates the narrator's output.
  const voiceKeys = ['narrator'];
  if (characterReplies) {
    for (const r of characterReplies) {
