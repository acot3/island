...
// biome's pool — see distributeScenes() in lib/map.js. minCount and maxCount
// per scene control how many copies may appear on a single map.
//
...
//   }
//   freshWater    boolean
//   items         list of authored items findable here. Each entry is
//                 { name, chance }. On a search, every remaining item is
//                 rolled independently — multiple finds per search are OK.
//                 Items are removed from the live pool once found (by name).
...
const BEACH_SCENES = [
  {
    id: 'wreckage-site',
    description: 'A wide stretch of pale sand. Inland, there is a jungle edge with trees and bushes. Large portions of a shipwreck have washed ashore. Debris and cargo is strewn about.',
    food: { baseChance: 1.0, kinds: ['canned peaches', 'bag of potato chips', 'protein bars'] },
    freshWater: true,
    items: [
      { name: 'hatchet', chance: 0.5 },
      { name: 'length of nylon rope', chance: 0.5 },
      { name: "captain's journal", chance: 0.5 },
    ],
    minCount: 1,
    maxCount: 1,
  },
  ...
  {
    id: 'tidepools',
    description: 'A wide stretch of pale sand strewn. Nearer the sea, rocky tidepools teem with life.',
    food: { baseChance: 0.8, kinds: ['shellfish', 'crabs', 'small fish'] },
    freshWater: false,
    items: [{ name: 'iridescent shell', chance: 0.5 }],
    minCount: 3,
    maxCount: 5,
  },
...
const JUNGLE_SCENES = [
  ...
  },
  {
    ...
    food: { baseChance: 0.2, kinds: ['fruit', 'edible leaves'] },
    freshWater: false,
    items: [{ name: 'skeleton key', chance: 0.5 }],
    minCount: 1,
    maxCount: 1,
  },
];
