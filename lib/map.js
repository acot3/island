...
function buildLocationPayload(room, name) {
  ...
  // Same fog rules as the host minimap, but coordinates are translated so
  // the player's current node sits at (0,0). Lets the phone render every
  // revealed node/edge while keeping tap targets driven by `neighbors`.
  const fogNodes = Object.entries(NODES)
    .filter(([id]) => seen.has(id))
    .map(([id, n]) => ({
      id, biome: n.biome,
      dx: n.x - cx, dy: n.y - cy,
  ...
  for (const [biome, nodeIds] of Object.entries(byBiome)) {
    ...
    nodeIds.forEach((nodeId, i) => {
      state[nodeId] = {
        ...
        annotations: [],    // short facts appended by events; treated as
                            // ground truth by narrator + categorizer.
      };
    });
  }
...
function getNodeView(room, nodeId) {
  ...
  return {
    ...
    annotations: ns.annotations ? ns.annotations.slice() : [],
  };
}

// Append a runtime annotation to a node — a short authored fact (e.g. a
// new permanent presence) that subsequent narration/categorization should
// treat as ground truth.
function annotateNode(room, nodeId, text) {
  if (!text) return false;
  const ns = room.nodeState && room.nodeState[nodeId];
  if (!ns) return false;
  if (!ns.annotations) ns.annotations = [];
  ns.annotations.push(text);
  return true;
}

// Replace the entire annotation list on a node — used when a stateful fact
// transitions (e.g. "chest is locked" → "chest is open") and the old
// statement would now contradict the new one. `texts` may be empty to
// clear all annotations.
function replaceNodeAnnotations(room, nodeId, texts) {
  const ns = room.nodeState && room.nodeState[nodeId];
  if (!ns) return false;
  ns.annotations = Array.isArray(texts) ? texts.slice() : [];
  return true;
}
