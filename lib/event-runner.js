...
const { annotateNode } = require('./map');
...
async function runEvent({ room, event, player, io }) {
  ...
  const engine = {
    ...
    async callStoryteller(instruction, { replace = false } = {}) {
      ...
      const segments = toolUse.input.segments.map((s) => {
      ...
      engine.emitBeat(segments, { replace });
    ...
    emitBeat(segments, { replace = false } = {}) {
      const item = { kind: 'beat', segments, replace };
      if (released) flushOne(item);
      else buffer.push(item);
    },
