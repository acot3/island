...
async function runEvent({ room, event, player, io }) {
  ...
  const engine = {
    ...
    async callStoryteller(instruction, { replace = false } = {}) {
      ...
      //
      // Defensive: the model occasionally returns `segments` as a non-array
      // (a string, or just missing). Fall back to a single narrator segment
      // from whatever text we can find rather than crashing the scene.
      ...
      if (Array.isArray(rawSegments)) {
        ...
        console.error(
          `[callStoryteller] segments not an array: ${JSON.stringify(toolUse.input).slice(0, 500)}`
        );
        const fallbackText = typeof rawSegments === 'string'
          ? rawSegments
          : (typeof toolUse.input.narration === 'string'
