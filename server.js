          ...
          if (allSubmitted && nonInjuredPlayers.length > 0) {
            ...
            try {
              ...
              if (response.ok) {
                ...
                // AUTOMATICALLY ADVANCE DAY after action resolution
                // This ensures health reduction, food/water consumption, and injury healing occur
                console.log(`[${roomCode}] Auto-advancing day after action resolution`);

                // Advance day counter
                room.currentDay += 1;

                // Apply daily health reduction (minimum 0)
                room.players.forEach(player => {
                  player.health = Math.max(0, player.health - 1);
                  // Handle injury persistence/clearing
                  if (player.injured && player.injuredOnDay !== null) {
                    if (room.currentDay > player.injuredOnDay + 1) {
                      player.injured = false;
                      player.injuredOnDay = null;
                ...
                // Food consumption: if enough food for all players, consume and restore 1 health
                const numPlayersForConsumption = room.players.length;
                if (room.food >= numPlayersForConsumption) {
                  room.food -= numPlayersForConsumption;
                  room.players.forEach(player => {
                    player.health = Math.min(10, player.health + 1); // Cap at 10
                  });
                }

                // Water consumption: if enough water, consume; otherwise reduce health by 1
                if (room.water >= numPlayersForConsumption) {
                  room.water -= numPlayersForConsumption;
                } else {
                  room.players.forEach(player => {
                    player.health = Math.max(0, player.health - 1);
                  });
                ...
                // Generate narration for new day
                let dayNarration = 'The sun rises on another day...';
                try {
                  if (!room.resourceStates) {
                    room.resourceStates = {};
                  }
                  ...
                  if (narrationResponse.ok) {
                    const narData = await narrationResponse.json();
                    dayNarration = narData.narration || dayNarration;

                    // Merge thread updates
                    if (!room.storyThreads) room.storyThreads = {};
                    const updates = Array.isArray(narData.threadUpdates) ? narData.threadUpdates : [];
                    ...
                    console.log(`[${roomCode}] Generated narration for day ${room.currentDay} after action resolution`);
                  }
                } catch (error) {
                  console.error('Error generating day narration after actions:', error.message || error);
                }

                // Emit day-advanced event to all players
                io.to(roomCode).emit('day-advanced', {
                  currentDay: room.currentDay,
                  players: room.players,
                  food: room.food,
